...
class Digest extends Injectable
{
    ...
    public function send()
    {
        ...
        $subject = sprintf('Top Stories %s', date('d/m/y'));
        ...
        foreach (Posts::find($parameters) as $i => $post) {
            ...
            $stories[$i]['user_url']     = "{$url}/user/{$user->login}";
            $stories[$i]['user_money']   = $user->getHumanMoney();
            ...
            $stories[$i]['post_url']     = "{$url}/post/{$post->id}";
        }
