...
class UploadHandler {
    ...
    public function handleUpload($uploadDirectory, $name = null){
        ...
        if (!is_null($this->sizeLimit) && $size > $this->sizeLimit) {
            return array('error' => '图片太大，图片大小不能超过500KB', 'preventRetry' => true);
        }
        ...
        if($this->allowedExtensions && !in_array(strtolower($ext), array_map("strtolower", $this->allowedExtensions))){
            ...
            return array('error' => '图片格式不正确，支持的图片格式：'. $these . '.');
        }
        ...
        # chunked upload
            ...
            # non-chunked upload
            $imgFolder = substr($name, 0, 3);

            $target = join(DIRECTORY_SEPARATOR, array($uploadDirectory, $imgFolder, $name));
            ...
            if ($target){
                ...
                if (move_uploaded_file($file['tmp_name'], $target)){
                    return array('success'=> true, "folder" => '/images/'.$imgFolder.'/');
                }
            }

            return array('error'=> '上传出错了，请重试');
        }
    }

    /**
     * Process the upload of app icons, qrcode, screenshots
     *
     * @param string $uploadDirectory Target directory.
     * @param string $name Overwrites the name of the file.
     */
    public function handleAppImageUpload($uploadDirectory, $appImageName, $app, $resizeInfo){
        ...
        // Validate name
        if ($name === null || $name === ''){
            return array('error' => 'Filename is empty');
        }

        // Validate file size
        if ($size == 0){
            return array('error' => 'Filename size is 0');
        }

        if (!is_null($this->sizeLimit) && $size > $this->sizeLimit) {
            return array('error' => '图片太大，图片大小不能超过1024KB', 'preventRetry' => true);
        }
        ...
        # non-chunked upload
            ...
            $stageImage = BASE_DIR.'public/stage/' . uniqid() . '.' . $ext;

            // Move the image to stage folder
            move_uploaded_file($file['tmp_name'], $stageImage);
            ...
            $result = true;
            try {
                $thumb->adaptiveResize($resizeInfo[0], $resizeInfo[1]);
                $thumb->save($app->imageFolder().'/'.$appImageName);
            } catch (Exception $e) {
                $result = false;
            }

            if ($appImageName == 'icon.png') {
                // Use PHPThumb GD genreate thumbnail
                $thumb2 = new GD($stageImage);
                try {
                    $thumb2->adaptiveResize(80, 80);
                    $thumb2->save($app->imageFolder().'/icon-small.png');
                } catch (Exception $e) {
                }
            }
